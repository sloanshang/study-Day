...
let pageIndex = 0;
class PregnancyCheck extends React.Component{
  ...
  constructor(props) {
    ...
    this.genData = () => {
      ...
      this.lastPage =page
      const url = config.BASIC_URL+'/testapi/pregnancycheck?_limit='+config.PAGE_SIZE+'&_page='+page
  ...
  render(){
    ...
    if(tabIndex==2) {
      if(isEdit){
        ...
        tabContent =<Form key={tabIndex}  onSubmit={this.handleSubmit}  initialValues={kv} data={editData} />
      }else{
       ...
       tabContent =<Form key={tabIndex}  onSubmit={this.handleSubmit}  initialValues={{f0:moment()}}  />
    }

    return <div>  <Header  title="妊检"/>
...
export default connect(mapStateToProps)(PregnancyCheck)
